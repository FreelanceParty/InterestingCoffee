...
<div class="flex flex-col gap-2">
	<div class="flex flex-col gap-2">
		<label class="flex flex-col gap-2 text-sm">
			Новий опис:
			<textarea name="description" rows="4" class="js-question-text px-2 w-full border-2 border-gray-200 rounded-lg text-lg resize-none">{{ $question->getText() }}</textarea>
		</label>
	</div>
			...
			sendRequest(
				...
				(response) => {
					if (response.ack === "success") {
						popup.showInfo("Питання оновлено!", '{{ InfoType::SUCCESS }}');
